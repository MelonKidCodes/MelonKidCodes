...
function increaseTime() {timetaken=Math.round((timetaken+0.1)*10)/10; document.getElementById("label").innerHTML = "Pick the button with the correct value! Time Taken: " + timetaken; document.getElementById("label2").innerHTML = "Questions answered: " + correctquestions + " Time/Question ratio: " + (timetaken / correctquestions);}

function doEquations() {
    correctbutton = (Math.random() > 0.75 ? "button1" : (Math.random() > 0.66 ? "button2" : (Math.random() > 0.5 ? "button3" : "button4")));
    let num1 = Math.round(Math.random() * 12) + 5;
    let num2 = Math.round(Math.random() * 12) + 5;
    for (i=0;i<4;i++) {
        let num = (Math.random() > 0.5 ? Math.round(num1 * num2 - Math.round(Math.random() * 4 + 2)) : Math.round(num1 * num2 + Math.round(Math.random() * 4 + 2)));
        document.getElementById("button" + (i + 1)).innerHTML = ("" + num1 + " * " + num2 + " = " + num);
    }
    let num = Math.round(num1 * num2);
    document.getElementById(correctbutton).innerHTML = ("" + num1 + " * " + num2 + " = " + num);
}
